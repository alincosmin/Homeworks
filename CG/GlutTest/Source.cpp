...
void drawPolygon(int sides, float variation, float red, float green, float blue)
{
	float angle;
	...
	{
		angle = 2 * PI * i / sides;
		x = (sin(angle)) / variation;
		y = (cos(angle)) / variation;
		glVertex2f(x, y);
	}
	glEnd(); 
}
...
void Display8() {
	...
	drawPolygon(6, 1.3, 0, 0, 1);
	drawPolygon(6, 1.8, 1, 0, 0);
	drawPolygon(6, 1.9, 1, 1, 1);
}
