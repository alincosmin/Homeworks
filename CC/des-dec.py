...
def cbc_decode(block,key_str):
		plain = ""
		vector = key_str
		for i in range(len(block)/16):
			dec = decode(block[i*16:i*16+16],key_str)
			plain += xor_hex(dec,vector)
			vector = block[i*16:i*16+16]
		return plain

if len(sys.argv) != 3: 
	print "Syntax: " + sys.argv[0] + " CIPHERTEXT KEY"
	exit()
	
if len(sys.argv[1])%16 != 0:
	print "Cipher must have the length of 8n Bytes!"
	exit()
	
if len(sys.argv[2]) > 8:
	print "Key can't be lenghtier than 8!"
	exit()
...
print "Plaintext:"
if len(cipher) != 16:
	plain = cbc_decode(cipher,key)
	for i in range(len(plain)/16):
			print "\t" + plain[(i*16):(i*16+16)]
else:
	plain = decode(cipher,key)
	print "\t" + plain

print ""
print "Plaintext (unhexed):"
